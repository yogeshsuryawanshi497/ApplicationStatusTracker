...
def ping_utility(time_min, config_file, csv_file):
	...
	if (len(df_error.index) > 0):
		for ind in df_error.index:
			...
			try:
				Collection = "IPS_EXCEPTION_LOG"
				connection.insert_record_in_table(error_log, Collection)
			except Exception as Inserterror:
				logging.error("Unable to insert data into {}".format(Collection))
			df.loc[ind, 'count'] = 0
...
def create_config(filename):
	try:
		...
		logging.debug("Connection is not created with MONGO DB while creating"
					  " configfile:", Error)


if __name__=="__main__":
	# Initiallising logging
	dtstr = datetime.now().strftime("%Y%m%d%H")
